...
import { ExternalLink, Camera, FileText, Clock, X, Maximize2, Code } from 'lucide-react';
        ...
        <div className="space-y-4">
            <div className="relative border-l-2 border-gray-700 ml-4 space-y-8 py-2">
                {chain.map((node, index) => {
                    const isMalicious = node.cti_data?.verdict === 'Malicious';

                    return (
                        <div key={index} className="relative pl-8">
                            {/* Dot on the timeline */}
                            <div
                                className={`absolute -left-[9px] top-4 w-4 h-4 rounded-full border-2 ${isMalicious ? 'bg-red-500 border-red-900' : 'bg-emerald-500 border-emerald-900'
                                    }`}
                            />
                                            ...
                                            <div>
                                                ...
                                                <div className="bg-black/30 rounded p-2 overflow-x-auto max-h-40 overflow-y-auto">
                                                    <pre className="text-xs text-gray-400 font-mono whitespace-pre-wrap break-words">
                                                        {node.body_summary.substring(0, 5000)}
                                                    </pre>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {summaryReport && index === chain.length - 1 && (
                                            <div>
                                                ...
                                                <div className="bg-blue-900/10 border border-blue-800 rounded p-3">
                                                    <p className="text-sm text-gray-300 whitespace-pre-line">{summaryReport}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
